...
const editCourseEndpoint = "/courses"
export async function EditCourse(req: EditCourseRequest): Promise<boolean> {
    let sessionId = GetSessionId()
    if (!sessionId || sessionId == "") {
        throw Error("метод не доступен, пользователь не авторизован")
    }
    let r = await DefaultClient.PostJSON(editCourseEndpoint, req, DefaultClient.UserBearerAuthHeader(sessionId))
    return r.ok
}
